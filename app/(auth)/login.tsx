...
export default function LoginScreen() {
  ...
  const handleLogin = () => {
    ...
    setTimeout(() => {
      ...
      if (email.toLowerCase().includes('firm')) {
        router.replace('/(firm)/dashboard'); 
      } else if (email.toLowerCase().includes('admin') || email.toLowerCase().includes('verify')) {
        router.replace('/(verifier)/scan-qr'); 
      } else {
        Alert.alert('Access Denied', 'Invalid User Role. Use "firm@test.com" or "verify@test.com"');
      }
    }, 1500);
  };
